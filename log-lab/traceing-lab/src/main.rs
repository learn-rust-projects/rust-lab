...
fn compute_operation(x: i32, y: i32) -> i32 {
    let span = span!(Level::DEBUG, "内部计算", x, y);
    let _enter = span.enter();
...
async fn async_tracing_example() {
    ...
    // 异步span - 在异步上下文中追踪
    let async_span = span!(Level::INFO, "异步操作", operation = "数据库查询");
    async_span.in_scope(|| {
        info!("开始异步操作");
    });
...
#[tracing::instrument(name = "数据库操作", skip(db_connection, query))]
...
#[tracing::instrument]
fn debug_span_fields_example(users: &[&str]) {
    // 使用?和%操作符来控制字段的显示
